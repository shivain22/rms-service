...
public class RmsUserGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all rmsUsers").get("/services/rmsservice/api/rms-users").headers(headersHttpAuthenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new rmsUser")
                        .post("/services/rmsservice/api/rms-users")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"externalUserId\": \"SAMPLE_TEXT\"" +
                                ...
                                ", \"profileImageUrl\": \"SAMPLE_TEXT\"" +
                                ", \"isActive\": null" +
                                ", \"lastSyncAt\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"syncStatus\": \"SAMPLE_TEXT\"" +
                                ", \"syncError\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_rmsUser_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(http("Get created rmsUser").get("/services/rmsservice${new_rmsUser_url}").headers(headersHttpAuthenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created rmsUser").delete("/services/rmsservice${new_rmsUser_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the RmsUser entity").exec(scn);
