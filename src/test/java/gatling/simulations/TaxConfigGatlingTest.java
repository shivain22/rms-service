...
public class TaxConfigGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all taxConfigs")
                    .get("/services/rmsservice/api/tax-configs")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new taxConfig")
                        .post("/services/rmsservice/api/tax-configs")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                ...
                                ", \"taxRate\": 0" +
                                ", \"taxType\": \"SAMPLE_TEXT\"" +
                                ", \"isApplicableToFood\": null" +
                                ", \"isApplicableToBeverage\": null" +
                                ", \"isApplicableToAlcohol\": null" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_taxConfig_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created taxConfig").get("/services/rmsservice${new_taxConfig_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created taxConfig").delete("/services/rmsservice${new_taxConfig_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TaxConfig entity").exec(scn);
