...
public class ShiftGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all shifts").get("/services/rmsservice/api/shifts").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new shift")
                        .post("/services/rmsservice/api/shifts")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"shiftName\": \"SAMPLE_TEXT\"" +
                                ", \"startTime\": null" +
                                ", \"endTime\": null" +
                                ", \"shiftDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"isActive\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_shift_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created shift").get("/services/rmsservice${new_shift_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created shift").delete("/services/rmsservice${new_shift_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Shift entity").exec(scn);
