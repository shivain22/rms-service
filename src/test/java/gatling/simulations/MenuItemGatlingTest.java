...
public class MenuItemGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all menuItems")
                    .get("/services/rmsservice/api/menu-items")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new menuItem")
                        .post("/services/rmsservice/api/menu-items")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                ...
                                ", \"code\": \"SAMPLE_TEXT\"" +
                                ", \"description\": null" +
                                ", \"itemType\": \"SAMPLE_TEXT\"" +
                                ", \"cuisineType\": \"SAMPLE_TEXT\"" +
                                ", \"isVegetarian\": null" +
                                ", \"isVegan\": null" +
                                ", \"isAlcoholic\": null" +
                                ", \"spiceLevel\": 0" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_menuItem_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created menuItem").get("/services/rmsservice${new_menuItem_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created menuItem").delete("/services/rmsservice${new_menuItem_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MenuItem entity").exec(scn);
