...
public class PaymentMethodGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all paymentMethods")
                    .get("/services/rmsservice/api/payment-methods")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new paymentMethod")
                        .post("/services/rmsservice/api/payment-methods")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"methodCode\": \"SAMPLE_TEXT\"" +
                                ", \"methodName\": \"SAMPLE_TEXT\"" +
                                ", \"description\": null" +
                                ", \"isActive\": null" +
                                "}"
                            )
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_paymentMethod_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created paymentMethod")
                            .get("/services/rmsservice${new_paymentMethod_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created paymentMethod")
                        .delete("/services/rmsservice${new_paymentMethod_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the PaymentMethod entity").exec(scn);
