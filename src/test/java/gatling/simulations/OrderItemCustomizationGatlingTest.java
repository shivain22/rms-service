...
/**
 * Performance test for the OrderItemCustomization entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.11.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class OrderItemCustomizationGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all orderItemCustomizations")
                    .get("/services/rmsservice/api/order-item-customizations")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new orderItemCustomization")
                        .post("/services/rmsservice/api/order-item-customizations")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"quantity\": 0" + ", \"unitPrice\": 0" + ", \"totalPrice\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_orderItemCustomization_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created orderItemCustomization")
                            .get("/services/rmsservice${new_orderItemCustomization_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created orderItemCustomization")
                        .delete("/services/rmsservice${new_orderItemCustomization_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the OrderItemCustomization entity").exec(scn);
