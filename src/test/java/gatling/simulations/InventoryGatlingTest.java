 ...
 */
public class InventoryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all inventories")
                    .get("/services/rmsservice/api/inventories")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new inventory")
                        .post("/services/rmsservice/api/inventories")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                ...
                .on(
                    exec(
                        http("Get created inventory").get("/services/rmsservice${new_inventory_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created inventory").delete("/services/rmsservice${new_inventory_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Inventory entity").exec(scn);
