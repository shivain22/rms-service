 ...
 */
public class CustomerGatlingTest extends Simulation {
        ...
        .on(
            exec(
                http("Get all customers")
                    .get("/services/rmsservice/api/customers")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new customer")
                        .post("/services/rmsservice/api/customers")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"customerCode\": \"SAMPLE_TEXT\"" +
                                ", \"phone\": \"SAMPLE_TEXT\"" +
                                ", \"email\": \"SAMPLE_TEXT\"" +
                ...
                .on(
                    exec(
                        http("Get created customer").get("/services/rmsservice${new_customer_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created customer").delete("/services/rmsservice${new_customer_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Customer entity").exec(scn);
