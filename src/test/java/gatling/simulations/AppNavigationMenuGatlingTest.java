...
public class AppNavigationMenuGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all appNavigationMenus")
                    .get("/services/rmsservice/api/app-navigation-menus")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new appNavigationMenu")
                        .post("/services/rmsservice/api/app-navigation-menus")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"menuCode\": \"SAMPLE_TEXT\"" +
                                ", \"menuName\": \"SAMPLE_TEXT\"" +
                                ", \"description\": null" +
                                ", \"menuType\": \"SAMPLE_TEXT\"" +
                                ", \"icon\": \"SAMPLE_TEXT\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_appNavigationMenu_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created appNavigationMenu")
                            .get("/services/rmsservice${new_appNavigationMenu_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created appNavigationMenu")
                        .delete("/services/rmsservice${new_appNavigationMenu_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AppNavigationMenu entity").exec(scn);
