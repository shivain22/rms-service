...
/**
 * Performance test for the TableWaiterAssignment entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.11.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class TableWaiterAssignmentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all tableWaiterAssignments")
                    .get("/services/rmsservice/api/table-waiter-assignments")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new tableWaiterAssignment")
                        .post("/services/rmsservice/api/table-waiter-assignments")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"assignmentDate\": \"2020-01-01T00:00:00.000Z\"" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_tableWaiterAssignment_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created tableWaiterAssignment")
                            .get("/services/rmsservice${new_tableWaiterAssignment_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created tableWaiterAssignment")
                        .delete("/services/rmsservice${new_tableWaiterAssignment_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TableWaiterAssignment entity").exec(scn);
