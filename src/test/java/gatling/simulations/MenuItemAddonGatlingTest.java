...
public class MenuItemAddonGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all menuItemAddons")
                    .get("/services/rmsservice/api/menu-item-addons")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new menuItemAddon")
                        .post("/services/rmsservice/api/menu-item-addons")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"addonName\": \"SAMPLE_TEXT\"" +
                                ", \"addonCode\": \"SAMPLE_TEXT\"" +
                                ", \"price\": 0" +
                                ", \"isActive\": null" +
                                "}"
                            )
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_menuItemAddon_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created menuItemAddon")
                            .get("/services/rmsservice${new_menuItemAddon_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created menuItemAddon")
                        .delete("/services/rmsservice${new_menuItemAddon_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MenuItemAddon entity").exec(scn);
