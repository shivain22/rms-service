...
public class OrderStatusHistoryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all orderStatusHistories")
                    .get("/services/rmsservice/api/order-status-histories")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new orderStatusHistory")
                        .post("/services/rmsservice/api/order-status-histories")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"previousStatus\": \"SAMPLE_TEXT\"" +
                                ", \"newStatus\": \"SAMPLE_TEXT\"" +
                                ", \"changedAt\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"changedBy\": \"SAMPLE_TEXT\"" +
                                ", \"notes\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_orderStatusHistory_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created orderStatusHistory")
                            .get("/services/rmsservice${new_orderStatusHistory_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created orderStatusHistory")
                        .delete("/services/rmsservice${new_orderStatusHistory_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the OrderStatusHistory entity").exec(scn);
