...
public class CustomerLoyaltyGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all customerLoyalties")
                    .get("/services/rmsservice/api/customer-loyalties")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new customerLoyalty")
                        .post("/services/rmsservice/api/customer-loyalties")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                ...
                                ", \"lastPointsEarnedAt\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"isActive\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_customerLoyalty_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created customerLoyalty")
                            .get("/services/rmsservice${new_customerLoyalty_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created customerLoyalty")
                        .delete("/services/rmsservice${new_customerLoyalty_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the CustomerLoyalty entity").exec(scn);
