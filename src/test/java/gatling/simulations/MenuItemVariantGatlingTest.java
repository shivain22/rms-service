...
public class MenuItemVariantGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all menuItemVariants")
                    .get("/services/rmsservice/api/menu-item-variants")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new menuItemVariant")
                        .post("/services/rmsservice/api/menu-item-variants")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"variantName\": \"SAMPLE_TEXT\"" +
                                ", \"variantCode\": \"SAMPLE_TEXT\"" +
                                ", \"priceModifier\": 0" +
                                ", \"isDefault\": null" +
                                ", \"displayOrder\": 0" +
                                ", \"isActive\": null" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_menuItemVariant_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created menuItemVariant")
                            .get("/services/rmsservice${new_menuItemVariant_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created menuItemVariant")
                        .delete("/services/rmsservice${new_menuItemVariant_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the MenuItemVariant entity").exec(scn);
