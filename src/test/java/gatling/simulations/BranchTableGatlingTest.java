...
public class BranchTableGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all branchTables")
                    .get("/services/rmsservice/api/branch-tables")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new branchTable")
                        .post("/services/rmsservice/api/branch-tables")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"tableNumber\": \"SAMPLE_TEXT\"" +
                                ", \"tableName\": \"SAMPLE_TEXT\"" +
                                ", \"capacity\": 0" +
                                ", \"floor\": \"SAMPLE_TEXT\"" +
                                ", \"section\": \"SAMPLE_TEXT\"" +
                                ", \"status\": \"SAMPLE_TEXT\"" +
                                ", \"qrCode\": \"SAMPLE_TEXT\"" +
                                ", \"qrCodeUrl\": \"SAMPLE_TEXT\"" +
                                ", \"isActive\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_branchTable_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created branchTable").get("/services/rmsservice${new_branchTable_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created branchTable")
                        .delete("/services/rmsservice${new_branchTable_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the BranchTable entity").exec(scn);
