...
public class UserSyncLogGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all userSyncLogs")
                    .get("/services/rmsservice/api/user-sync-logs")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new userSyncLog")
                        .post("/services/rmsservice/api/user-sync-logs")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"syncType\": \"SAMPLE_TEXT\"" +
                                ...
                                ", \"syncedBy\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_userSyncLog_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created userSyncLog").get("/services/rmsservice${new_userSyncLog_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created userSyncLog")
                        .delete("/services/rmsservice${new_userSyncLog_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the UserSyncLog entity").exec(scn);
