 ...
 */
public class DiscountGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all discounts")
                    .get("/services/rmsservice/api/discounts")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new discount")
                        .post("/services/rmsservice/api/discounts")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"discountCode\": \"SAMPLE_TEXT\"" +
                                ", \"discountName\": \"SAMPLE_TEXT\"" +
                                ", \"discountType\": \"SAMPLE_TEXT\"" +
                                ", \"discountValue\": 0" +
                                ", \"minOrderAmount\": 0" +
                                ", \"maxDiscountAmount\": 0" +
                                ", \"applicableTo\": \"SAMPLE_TEXT\"" +
                ...
                .on(
                    exec(
                        http("Get created discount").get("/services/rmsservice${new_discount_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created discount").delete("/services/rmsservice${new_discount_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Discount entity").exec(scn);
