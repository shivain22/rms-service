...
public class BillDiscountGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all billDiscounts")
                    .get("/services/rmsservice/api/bill-discounts")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new billDiscount")
                        .post("/services/rmsservice/api/bill-discounts")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"discountCode\": \"SAMPLE_TEXT\"" +
                                ", \"discountType\": \"SAMPLE_TEXT\"" +
                                ", \"discountValue\": 0" +
                                ", \"discountAmount\": 0" +
                                "}"
                            )
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_billDiscount_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created billDiscount")
                            .get("/services/rmsservice${new_billDiscount_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created billDiscount")
                        .delete("/services/rmsservice${new_billDiscount_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the BillDiscount entity").exec(scn);
