...
public class BillItemGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all billItems")
                    .get("/services/rmsservice/api/bill-items")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new billItem")
                        .post("/services/rmsservice/api/bill-items")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_billItem_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created billItem").get("/services/rmsservice${new_billItem_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created billItem").delete("/services/rmsservice${new_billItem_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the BillItem entity").exec(scn);
