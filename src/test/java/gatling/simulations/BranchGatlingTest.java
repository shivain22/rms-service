 ...
 */
public class BranchGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all branches").get("/services/rmsservice/api/branches").headers(headersHttpAuthenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new branch")
                        .post("/services/rmsservice/api/branches")
                ...
                .repeat(5)
                .on(
                    exec(http("Get created branch").get("/services/rmsservice${new_branch_url}").headers(headersHttpAuthenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created branch").delete("/services/rmsservice${new_branch_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Branch entity").exec(scn);
