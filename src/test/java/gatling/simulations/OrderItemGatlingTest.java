 ...
 */
public class OrderItemGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all orderItems")
                    .get("/services/rmsservice/api/order-items")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new orderItem")
                        .post("/services/rmsservice/api/order-items")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_orderItem_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created orderItem").get("/services/rmsservice${new_orderItem_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created orderItem").delete("/services/rmsservice${new_orderItem_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the OrderItem entity").exec(scn);
