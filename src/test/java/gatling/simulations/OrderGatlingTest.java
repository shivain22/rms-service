...
public class OrderGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all orders").get("/services/rmsservice/api/orders").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new order")
                        .post("/services/rmsservice/api/orders")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"orderNumber\": \"SAMPLE_TEXT\"" +
                                ", \"orderType\": \"SAMPLE_TEXT\"" +
                                ", \"orderSource\": \"SAMPLE_TEXT\"" +
                                ", \"status\": \"SAMPLE_TEXT\"" +
                                ", \"orderDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"estimatedReadyTime\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"actualReadyTime\": \"2020-01-01T00:00:00.000Z\"" +
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created order").get("/services/rmsservice${new_order_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created order").delete("/services/rmsservice${new_order_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );
