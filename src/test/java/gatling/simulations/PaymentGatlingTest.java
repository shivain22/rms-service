 ...
 */
public class PaymentGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all payments").get("/services/rmsservice/api/payments").headers(headersHttpAuthenticated).check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new payment")
                        .post("/services/rmsservice/api/payments")
                ...
                .repeat(5)
                .on(
                    exec(http("Get created payment").get("/services/rmsservice${new_payment_url}").headers(headersHttpAuthenticated)).pause(
                        10
                    )
                )
                .exec(http("Delete created payment").delete("/services/rmsservice${new_payment_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Payment entity").exec(scn);
