...
/**
 * Performance test for the AppNavigationMenuRole entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.11.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class AppNavigationMenuRoleGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all appNavigationMenuRoles")
                    .get("/services/rmsservice/api/app-navigation-menu-roles")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new appNavigationMenuRole")
                        .post("/services/rmsservice/api/app-navigation-menu-roles")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"role\": \"SAMPLE_TEXT\"" + ", \"isActive\": null" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_appNavigationMenuRole_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created appNavigationMenuRole")
                            .get("/services/rmsservice${new_appNavigationMenuRole_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created appNavigationMenuRole")
                        .delete("/services/rmsservice${new_appNavigationMenuRole_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AppNavigationMenuRole entity").exec(scn);
