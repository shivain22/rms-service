...
/**
 * Performance test for the AppNavigationMenuItem entity.
 *
 * @see <a href="https://github.com/jhipster/generator-jhipster/tree/v8.11.0/generators/gatling#logging-tips">Logging tips</a>
 */
public class AppNavigationMenuItemGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .on(
            exec(
                http("Get all appNavigationMenuItems")
                    .get("/services/rmsservice/api/app-navigation-menu-items")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new appNavigationMenuItem")
                        .post("/services/rmsservice/api/app-navigation-menu-items")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                ...
                                ", \"description\": null" +
                                ", \"icon\": \"SAMPLE_TEXT\"" +
                                ", \"routePath\": \"SAMPLE_TEXT\"" +
                                ", \"componentPath\": \"SAMPLE_TEXT\"" +
                                ", \"displayOrder\": 0" +
                                ", \"badgeText\": \"SAMPLE_TEXT\"" +
                                ", \"badgeColor\": \"SAMPLE_TEXT\"" +
                                ", \"isActive\": null" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_appNavigationMenuItem_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created appNavigationMenuItem")
                            .get("/services/rmsservice${new_appNavigationMenuItem_url}")
                            .headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(
                    http("Delete created appNavigationMenuItem")
                        .delete("/services/rmsservice${new_appNavigationMenuItem_url}")
                        .headers(headersHttpAuthenticated)
                )
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the AppNavigationMenuItem entity").exec(scn);
