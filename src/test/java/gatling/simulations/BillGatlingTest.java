...
public class BillGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(
        ...
        .repeat(2)
        .on(
            exec(http("Get all bills").get("/services/rmsservice/api/bills").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new bill")
                        .post("/services/rmsservice/api/bills")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                                "\"billNumber\": \"SAMPLE_TEXT\"" +
                                ", \"billDate\": \"2020-01-01T00:00:00.000Z\"" +
                                ", \"subtotal\": 0" +
                                ", \"taxAmount\": 0" +
                                ", \"discountAmount\": 0" +
                                ", \"serviceCharge\": 0" +
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created bill").get("/services/rmsservice${new_bill_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created bill").delete("/services/rmsservice${new_bill_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Bill entity").exec(scn);
